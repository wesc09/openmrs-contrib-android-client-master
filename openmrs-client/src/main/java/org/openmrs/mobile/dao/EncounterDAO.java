...
public class EncounterDAO {
    ...
    public void saveLastAdmissionEncounter(Encounter encounter, String patientUUID) {
        if (null != encounter) {
            encounter.setPatientUUID(patientUUID);
            long oldLastAdmissionEncounterID = getLastAdmissionEncounterID(patientUUID);
            if (0 != oldLastAdmissionEncounterID) {
                for (Observation obs: new ObservationDAO().findObservationByEncounterID(oldLastAdmissionEncounterID)) {
                    new ObservationTable().delete(obs.getId());
                }
                new EncounterTable().delete(oldLastAdmissionEncounterID);
            }
            ObservationDAO observationDAO = new ObservationDAO();
            long encounterID = saveEncounter(encounter, null);
            for (Observation obs : encounter.getObservations()) {
    ...
    public long getLastAdmissionEncounterID(String patientUUID) {
        long encounterID = 0;
        DBOpenHelper helper = OpenMRSDBOpenHelper.getInstance().getDBOpenHelper();
        String where = String.format("%s is NULL AND %s = ?", EncounterTable.Column.VISIT_KEY_ID, EncounterTable.Column.PATIENT_UUID);
        String[] whereArgs = new String[]{patientUUID};
