...
public class VisitApi {
    ...
    public void syncLastAdmission(final String patientUuid) {
        syncLastAdmission(patientUuid, null);
    }
    ...
    public void syncLastAdmission(final String patientUuid, @Nullable final DefaultResponseCallbackListener callbackListener) {
        Call<Results<Encounter>> call = restApi.getLastAdmission(patientUuid, ApplicationConstants.EncounterTypes.ADMISSION, "full", 1,"desc");
        call.enqueue(new Callback<Results<Encounter>>() {
            @Override
            public void onResponse(Call<Results<Encounter>> call, Response<Results<Encounter>> response) {
                if (response.isSuccessful()) {
                    if (!response.body().getResults().isEmpty()) {
                        encounterDAO.saveLastAdmissionEncounter(response.body().getResults().get(0), patientUuid);
                    }
                    if (callbackListener != null) {
                        callbackListener.onResponse();
                    }
